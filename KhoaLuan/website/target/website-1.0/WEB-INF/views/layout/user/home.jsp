...
<c:if test="${infoLivestream.size()>0}"> 
    <section class="latest-product spad">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="section-title">
                        <h2>Các phiên phát sóng hiện tại</h2>
                    </div>

                </div>
            </div>

            <c:forEach items="${infoLivestream}" var="item">
                <a href="<c:url value="/viewlive?userlive="/>${item.getKey()}" style="text-decoration: none">
                    <div class="card">

                        <div class="card-body">
                            <p class="card-text">
                                <i class="fa fa-bullseye" aria-hidden="true" style="color: red"></i>
                                <b class="card-title">Livestream của ${item.getKey()}</b> - ${item.getValue().content}.
                                <cite class="float-right" title="Source Title">Time: ${item.getValue().time.toString()}</cite>
                            </p>
                        </div>
                    </div>
                </a>
